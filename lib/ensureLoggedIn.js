...
module.exports = function ensureLoggedIn(options) {
  ...
  return function(req, res, next) {
    if (!req.isAuthenticated || !req.isAuthenticated()) {
      return handleUnauthenticated(req, res, options, url, setReturnTo);
    }
    next();
  }
}

function handleUnauthenticated(req, res, options, url, setReturnTo) {
  if (options.unauthenticatedHandler) {
    options.unauthenticatedHandler(req, res);
    return;
  }
